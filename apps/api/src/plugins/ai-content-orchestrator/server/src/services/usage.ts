...
const usage = ({ strapi }: { strapi: Strapi }) => {
  ...
  async assertBudget(input: BudgetCheckInput): Promise<{ blocked: boolean; usage: UsageDailyRecord }> {
  ...
  async registerUsage(workflowId: number, day: string, usageStats: OpenRouterUsage): Promise<UsageDailyRecord> {
    ...
    const updated = (await entityService.update(USAGE_DAILY_UID, usageRecord.id, {
      data: {
        ...
        prompt_tokens: Number(usageRecord.prompt_tokens ?? 0) + Number(usageStats.prompt_tokens ?? 0),
        completion_tokens:
          Number(usageRecord.completion_tokens ?? 0) + Number(usageStats.completion_tokens ?? 0),
        total_tokens: Number(usageRecord.total_tokens ?? 0) + Number(usageStats.total_tokens ?? 0),
      },
    })) as UsageDailyRecord;
