...
export class TarotService {
  ...
  public getDailyCard(): Observable<DailyTarotDrawResponse> {
    return this.http
      .get<DailyTarotDrawResponse>(
        `${this.apiUrl}/daily-tarot/today?populate[card][populate]=image`,
      )
  ...
  public getCardBySlug(slug: string): Observable<TarotCard | undefined> {
    return this.http
      .get<
        StrapiCollectionResponse<TarotCard>
      >(`${this.apiUrl}/tarot-cards?filters[slug][$eq]=${encodeURIComponent(slug)}&populate=image`)
      .pipe(
