...
export class NewsletterService {
  ...
  public confirm(token: string): Observable<NewsletterActionResponse> {
    return this.http.get<NewsletterActionResponse>(
      `${this.apiUrl}/newsletter/confirm/${encodeURIComponent(token)}`,
    );
  }

  public unsubscribe(token: string): Observable<NewsletterActionResponse> {
    return this.http.get<NewsletterActionResponse>(
      `${this.apiUrl}/newsletter/unsubscribe/${encodeURIComponent(token)}`,
    );
  }
}
